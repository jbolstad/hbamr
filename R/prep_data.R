...
#' @param allow_miss Integer specifying how many missing stimulus positions should be accepted for an individual still to be included in the analysis. Defaults to 2.
...
#' @param req_unique Integer specifying how many unique positions on the ideological scale each respondent is required to have used when placing the stimuli in order to be included in the analysis. The default is `req_unique = 2`.
...
#' @param quiet Logical: Should information about the data be printed to the console? Defaults to `FALSE`.
...
prep_data <- function(self = NULL, stimuli,
  ...
  datlist <- list(J = ncol(stimuli), N = nrow(stimuli), B = B, B2 = ceiling(B), N_obs = length(stimuli_vec),
       V = self, Vi = floor(self) + ceiling(B) + 1, Y = stimuli_vec, U = prefs_vec, L = L, R = R, K = range_orig[2] - range_orig[1] + 1,
